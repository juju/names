...
var caasApplicationNameTests = []struct {
	pattern string
	valid   bool
}{
	{pattern: "", valid: false},
...
func (s *caasApplicationSuite) TestCAASApplicationNameFormats(c *gc.C) {
	assertCAASApplication := func(s string, expect bool) {
		c.Assert(names.IsValidCAASApplication(s), gc.Equals, expect)
	...
	for i, test := range caasApplicationNameTests {
		c.Logf("test %d: %q", i, test.pattern)
		assertCAASApplication(test.pattern, test.valid)
	}
}

var parseCAASApplicationTagTests = []struct {
	tag      string
	expected names.Tag
	err      error
}{{
	...
	tag:      "caasapplication-dave",
	expected: names.NewCAASApplicationTag("dave"),
}, {
	tag: "dave",
	err: names.InvalidTagError("dave", ""),
}, {
	tag: "caasapplication-dave/0",
	err: names.InvalidTagError("caasapplication-dave/0", names.CAASApplicationTagKind),
}, {
	tag: "caasapplication",
	err: names.InvalidTagError("caasapplication", ""),
}, {
	tag: "user-dave",
	err: names.InvalidTagError("user-dave", names.CAASApplicationTagKind),
}}

func (s *caasApplicationSuite) TestParseCAASApplicationTag(c *gc.C) {
	for i, t := range parseCAASApplicationTagTests {
		c.Logf("test %d: %s", i, t.tag)
		got, err := names.ParseCAASApplicationTag(t.tag)
